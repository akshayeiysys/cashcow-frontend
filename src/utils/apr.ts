...
export const getFarmApr = (poolWeight: BigNumber, cakePriceUsd: BigNumber, poolLiquidityUsd: BigNumber): number => {
  ...
  //    apr = yearlyCakeRewardAllocation.times(cakePriceUsd).div(poolLiquidityUsd).times(100)
  // }else
  // {
    const apr = yearlyCakeRewardAllocation.times(cakePriceUsd)
  // }
  
  console.log(apr,yearlyCakeRewardAllocation,cakePriceUsd,poolLiquidityUsd,"apr,yearlyCakeRewardAllocation,cakePriceUsd,poolLiquidityUsd")
  return apr.isNaN() || !apr.isFinite() ? null : apr.toNumber()
}
